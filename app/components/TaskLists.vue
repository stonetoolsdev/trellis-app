...
const taskListEndpoint = computed(() =>
  props.contextType === 'event'
    ? `/api/v1/events/${props.contextId}/task-lists`
    : `/api/v1/projects/${props.contextId}/task-lists`
)

async function handleAddList() {
  if (!newListTitle.value.trim()) return
  await request(taskListEndpoint.value, {
    method: 'POST',
...
async function handleAddTask(listId: string) {
  ...
  await request('/api/v1/tasks', {
    ...
    body: {
      ...
      [`${props.contextType}_id`]: props.contextId,
      task_list_id: listId,
    },
  })
  newTaskTitle.value = ''
    ...
    <div v-for="list in taskLists" :key="list.id" class="space-y-2">
      ...
      <div class="space-y-1">
        <div v-for="task in list.tasks" :key="task.id"
          class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-accent/50 transition-colors cursor-pointer"
          @click="openTask(task)">

...
import { CalendarDays, FolderKanban, CheckSquare, ShoppingBasket, UserRoundCog, CalendarCog, Drama, FileText } from 'lucide-vue-next'
...
const navItems = [
  ...
  { label: 'Notes', icon: FileText, to: '/notes', children: null },
  {
    label: 'Events',
    icon: CalendarDays,
    to: '/events',
...
<div class="flex-1 py-3 space-y-0.5">
  ...
  <nav v-if="user?.roles?.includes('admin')" class="px-2">
    <h3 v-if="sidebarOpen"
      class="uppercase text-xs font-semibold my-4 ml-2 ">
      Admin</h3>
